...
The driver session (row `driver @local`, ⌘⌃D) starts and watches other sessions. Its cwd is `$WT_REPOS_DIR`, which is not a repo, so every `wt new` needs `-r <repo>`. `wt` hides local/remote: `wt <sub> …` for this Mac, `wt -H <vm> <sub> …` for a VM.
...
3. **Report each task**: name, row title (`<repo>:<slug> @local` or `@<vm>`), branch, path, and on a VM the tmux session.
